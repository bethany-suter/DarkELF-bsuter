...
##############################################################################
# Calculates the C_ld's using both multiphonon expansion and impulse approximation. Uses the Fn(omega) files
...
def C_ld(self, qrange, omega, d):
    ...
    if q_multiphonon.size == 0:
        ...
        print('no multiphonon')
    else:
        # cld_multiphonon = np.sum(list(map(lambda n: (1/(2*self.Avec[d]*self.mp))**n * q_multiphonon**(2*n) * self.debye_waller(q_multiphonon).T[d] * self.Fn_interpolations[d][n](omega), range(1, len(self.phonon_Fn[d])))), axis=0)
